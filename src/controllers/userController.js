...
export const userDelete = async (req, res) => {
  try {
    ...
    if (!searchUser) {
      return res
        .status(404)
        .json({ mensaje: "El usuario selicitado no existe" });
    }
...
export const userEdit = async (req, res) => {
  try {
    ...
    if (!updatedUser) {
      return res
        .status(404)
        .json({ mensaje: "El usuario solicitado no existe" });
    }
    ...
    res
      .status(500)
      .json({ mensaje: "Ocurrió un error, no se pudo actualizar el usuario" });
  }
};
